...
public abstract class StorageEngineTestSuite<ESE extends EventStorageEngine> {
    ...
    @Test
    @Disabled  // TODO #15 Investigate flakey test in StorageEngineTestSuite (revert to using the AF StorageEgineTestSuite once resolved)
    void concurrentTransactionsForNonOverlappingTagsBothCommitWithExpectedConsistencyMarkerResponse() throws Exception {
        ...
        AppendCondition firstCondition = AppendCondition.withCriteria(TEST_CRITERIA).withMarker(ConsistencyMarker.ORIGIN);
        AppendCondition secondCondition = AppendCondition.withCriteria(OTHER_CRITERIA).withMarker(ConsistencyMarker.ORIGIN);
    ...
    void concurrentTransactionsForOverlappingTagsThrowAnAppendEventsTransactionRejectedException() throws Exception {
        ...
        AppendCondition firstCondition = AppendCondition.withCriteria(TEST_CRITERIA).withMarker(ConsistencyMarker.ORIGIN);
        AppendCondition secondCondition = AppendCondition.withCriteria(TEST_CRITERIA).withMarker(ConsistencyMarker.ORIGIN);
        CompletableFuture<AppendTransaction<?>> firstTx =
                testSubject.appendEvents(firstCondition, processingContext(), taggedEventMessage("event-0", TEST_CRITERIA_TAGS));
        CompletableFuture<AppendTransaction<?>> secondTx =
